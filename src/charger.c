...
{
    while (1) {
        /* The charger shares i2c0 with the sensor, and the bus is suspended
         * while the device is in standby. Only poll when the bus is up. */
        if ((power_status == PWR_ON || power_status == PWR_HOLD) &&
            i2c_bus_lock(K_MSEC(50)) == 0) {
            log_charger_status();
            i2c_bus_unlock();
        }
